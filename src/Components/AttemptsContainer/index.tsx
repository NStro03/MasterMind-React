...
function AttemptsContainer(props: { boardColors: Array<Array<string>>, boardHints: Array<Array<string>>, attemptsMade: number, action: Function, selectedColor: string }) {
// Use Map for each element in boardColors
    const attemptComponents = props.boardColors.map((iterAttempt: Array<string>, index: number) => 
        <Attempt isActive={props.attemptsMade === index ? true : false} action={props.action} attemptColors={iterAttempt} attemptHints={Array<string>(...props.boardHints[index])} key={index} />
    )
